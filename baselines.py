...
class POP():
    ...
    def compute_rank(self,data,store,k=10):
        ...
        for b in range(pos.shape[0]): 
            avt = pos[b,:-1]
            avt = avt[avt!=0]
            mask[b] = pos[b,-1] in avt # whether the target item is in the history
            store['ratio'] += [float(pos[b,-1] in avt)]
            
        for b in range(inputs.shape[0]):
            step = xtsy[b,-1].item()
            av = self.args.ts[step] # available steamers in target time
            
            scores = np.array([self.cnt[a] for a in av]) # scores for each available streamer
            iseq = pos[b,-1] == torch.LongTensor(av).to(self.args.device) 
            idx = torch.where(iseq) 
            idx = int(idx[0].item())# find target streamer index in available streamers list
            rank = np.where(scores.argsort()[::-1]==idx)# find the target streamer's ranking among all available streamers
