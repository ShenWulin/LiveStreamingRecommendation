...
def get_model_type_new(args):
    ...
    if args.model == "POP":
        ...
        MODEL = FPMC
    elif args.model == "LiveRec_dur_emb_v2":
        MODEL = LiveRec_dur_emb_v2
...
class LiveRec_dur_emb_v2(nn.Module):
    def __init__(self, args):
        super(LiveRec_dur_emb_v2, self).__init__()
        self.args = args
    ...
    def forward(self, log_seqs,dur_seqs):
...
#         i_embs = 0
    ...
    def compute_rank(self,data,store,k=10):
        ...
        feats_1,feats_2 = self(inputs,dur)
       
        # Add time interval embeddings 
        if self.args.fr_ctx:
            ctx,batch_inds = self.get_ctx_att(data,feats_1)
    ...
    def get_ctx_att(self,data,feats,neg=None):
        ...
        flat_feats = feats[ci[:,0],ci[:,1],:]  # -1 args.K
        flat_feats = flat_feats.unsqueeze(1).expand(flat_feats.shape[0], self.args.av_tens.shape[-1], flat_feats.shape[1]) # -1 max_av args.K
    ...
    def train_step(self, data, use_ctx=False): # for training
        inputs,pos,dur = data[:,:,3],data[:,:,5],data[:,:,7]
        ...
        if self.args.fr_ctx:
            ctx_pos,ctx_neg = self.get_ctx_att(data,feats_1,neg)
