...
def get_model_type_new(args):
    ...
    if args.model == "POP":
        ...
        MODEL = FPMC
    elif args.model == "LiveRec_dur_lstm_v2":
        MODEL = LiveRec_dur_lstm_v2
...
class Attention(nn.Module): # Attention layer
    def __init__(self, args, num_att, num_heads, causality=False):
        ...
        for _ in range(num_att):
            ...
            new_attn_layer =  nn.MultiheadAttention(args.K,num_heads,0.2)
...
class LiveRec_dur_lstm_v2(nn.Module):
    def __init__(self, args):
        super(LiveRec_dur_lstm_v2, self).__init__()
        self.args = args
        ...
        self.time_emb = nn.Embedding(args.T+1, args.K, padding_idx=0)
        
        self.emb_dropout = nn.Dropout(p=0.2)

        # Sequence encoding attention
        self.att = Attention(args, args.num_att, args.num_heads, causality=True)

        # Availability attention
        self.att_ctx = Attention(args, args.num_att_ctx, args.num_heads_ctx, causality=False)
        ...
        self.lstm = nn.LSTM(args.K * 2, args.K, batch_first=True, bidirectional=False)
 

    def forward(self,log_seqs,dur_seqs,times_seqs):   
#     def forward(self, log_seqs):
#     def forward(self,log_seqs,dur_seqs,dur_diff_seqs):  # add duration information
        seqs_1 = self.item_embedding(log_seqs)
        seqs_1 *= self.item_embedding.embedding_dim ** 0.5 # ???
        
        positions = np.tile(np.array(range(log_seqs.shape[1])), [log_seqs.shape[0], 1])
        seqs_1 += self.pos_emb(torch.LongTensor(positions).to(self.args.device)) #position embedding
        seqs_1 += self.dur_emb(dur_seqs) # duration embedding
...
#         seqs_2 += self.dur_emb(dur_seqs) # duration embedding
        seqs_2 = self.emb_dropout(seqs_2)
        
#         seqs_2 = self.att(seqs_2, timeline_mask)
#         feats_2 = self.att(seqs_2, timeline_mask)
        
        times_embs = self.time_emb(times_seqs)
        seqs_2 = torch.cat([seqs_2, times_embs], dim=-1).transpose(0, 1)
#         seqs_2 = torch.cat([seqs_2, times_embs], dim=-1)
        
        feats_2,_ = self.lstm(seqs_2)
        feats_2 = feats_2.transpose(0, 1)
        
        return feats_1,feats_2


    def predict(self,feats_1,feats_2,inputs,items,ctx,data):
#         i_embs = 0
        if ctx!=None: 
            i_embs = ctx 
        else:  
...
#         feats_1 = self(inputs,dur,time)
        ...
        if self.args.fr_ctx:
            ctx,batch_inds = self.get_ctx_att(data,feats_1,feats_2)
    ...
    def get_ctx_att(self,data,feats_1,feats_2,neg=None):
        ...
        flat_xtsy = xtsy[ci[:,0],ci[:,1]] # 1 dimension

        av = self.args.av_tens[flat_xtsy,:]
        av_embs = self.item_embedding(av) # -1 max_av args.K

        # repeat consumption: time interval embeddings
        if self.args.r_rep:
            av_rep_batch = self.get_av_rep(data)
            av_rep_flat  = av_rep_batch[ci[:,0],ci[:,1]]
            rep_enc = self.rep_emb(av_rep_flat) # time interval embedding
            av_embs += rep_enc   

        flat_feats_1 = feats_1[ci[:,0],ci[:,1],:]  # -1 args.K
        flat_feats_1 = flat_feats_1.unsqueeze(1).expand(flat_feats_1.shape[0], self.args.av_tens.shape[-1], flat_feats_1.shape[1]) # -1 max_av args.K
        
#         flat_feats_2 = feats_2[ci[:,0],ci[:,1],:]  # -1 args.K
#         flat_feats_2 = flat_feats_2.unsqueeze(1).expand(flat_feats_2.shape[0], self.args.av_tens.shape[-1], flat_feats_2.shape[1]) # -1 max_av args.K
        
#         scores = (av_embs * flat_feats_1 + av_embs * flat_feats_2).sum(-1)
        scores = (av_embs * flat_feats_1).sum(-1)
        ...
        if pos != None and neg != None:
            return expand_att(pos),expand_att(neg) # there is a problem
        # testing
        else:
    ...
    def train_step(self, data, use_ctx=False): # for training
        time,inputs,pos,dur = data[:,:,2],data[:,:,3],data[:,:,5],data[:,:,7]
        neg = sample_negs(data,self.args).to(self.args.device)

        feats_1,feats_2 = self(inputs,dur,time) # get sequence embedding
      
        ctx_pos,ctx_neg = None,None
        if self.args.fr_ctx:
            ctx_pos,ctx_neg = self.get_ctx_att(data,feats_1,feats_2,neg)
            
        pos_logits = self.predict(feats_1,feats_2,inputs,pos,ctx_pos,data) 
        neg_logits = self.predict(feats_1,feats_2,inputs,neg,ctx_neg,data) 
    ...
    def get_av_rep(self,data): #too complicated
        bs     = data.shape[0] #batch
        inputs = data[:,:,3] # inputs 
        xtsb   = data[:,:,2] # inputs ts
        xtsy   = data[:,:,6] # targets ts   ba seq_len

        av_batch  = self.args.av_tens[xtsy.view(-1),:]
        av_batch  = av_batch.view(xtsy.shape[0],xtsy.shape[1],-1) # bs seq_len max_av
        av_batch *= (xtsy!=0).unsqueeze(2) # masking pad inputs
        av_batch  = av_batch.to(self.args.device)

        mask_caus = 1-torch.tril(torch.ones(self.args.seq_len,self.args.seq_len),diagonal=-1) # seq_len seq_len
        mask_caus = mask_caus.unsqueeze(0).unsqueeze(3) # 1 seq_len seq_len 1
        mask_caus = mask_caus.expand(bs,-1,-1,self.args.av_tens.shape[-1]) #  bs  seq_len seq_len max_av
        ...
        bm   = (inputs.unsqueeze(2).unsqueeze(3)==av_batch.unsqueeze(1).expand(-1,self.args.seq_len,-1,-1)) # it is hard to figure out the data structure
        bm  &= mask_caus
        ...
        sm   = bm.type(torch.int).argmax(1) # find the first index
