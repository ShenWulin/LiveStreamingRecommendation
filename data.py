...
def load_data(args):
    INFILE = os.path.join(args.dataset,'douyu_10k.csv')
    #user,stream,streamer_id,start,stop
#     cols = ["user","streamer","start","stop"]
...
def get_dataloaders(data_fu, args):
    ...
    cache_tr = os.path.join(args.cache_dir,"tr.p")
    cache_te = os.path.join(args.cache_dir,"te.p")
    cache_va = os.path.join(args.cache_dir,"val.p")
